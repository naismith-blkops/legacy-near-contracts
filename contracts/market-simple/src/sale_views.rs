...
impl Contract {
    ...
    pub fn get_sales_by_owner_id(
        ...
        keys.iter()
            ...
            //we'll map the sale IDs into Sale objects
            .map(|sale_ids| self.sales.get(&sale_ids).unwrap())
    ...
    //get the number of sales for a specific token type. (returns a string)
    pub fn get_supply_by_nft_token_type(
        &self,
        token_type: String,
    ) -> U64 {
        //get the set of tokens for associated with the given token type
    ...
    //returns paginated sale objects associated with a given token type. (result is a vector of sales)
    pub fn get_sales_by_nft_token_type(
        ...
        //get the set of Sale IDs for the given token type
        let by_nft_token_type = self.by_nft_token_type.get(&token_type);
